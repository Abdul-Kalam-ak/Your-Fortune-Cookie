...
(() => {
  ...
  function finishCrack(skipShake) {
    ...
    setTimeout(() => {
      ...
      // Analytics: log a "crack" event (separate from page views), tagged
      // with which moment/preference path led here.
      if (window.goatcounter && window.goatcounter.count) {
        window.goatcounter.count({
          path: `crack-${state.moment}-${state.preference}`,
          title: "Cookie Cracked",
          event: true,
        });
      }
